...
#
#   Derived from meta-boot2qt/scripts/setup-environment.sh
#
...
if [ ! -e ${PWD}/${BUILDDIRECTORY} ]; then
  ...
  LAYERSCONF=${PWD}/$TEMPLATES/${LAYERSCONF}
  ...
  if [ ! -e "${PWD}/$TEMPLATES/local.conf.sample" ]; then
    cp ${PWD}/meta-boot2qt/meta-boot2qt-distro/conf/local.conf.sample  ${PWD}/${BUILDDIRECTORY}/conf/local.conf
  fi

  if [ -e ${PWD}/meta-boot2qt/.QT-FOR-DEVICE-CREATION-LICENSE-AGREEMENT ]; then
    QT_SDK_PATH=$(readlink -f ${PWD}/meta-boot2qt/../../../)
  fi
fi

export TEMPLATECONF="${PWD}/$TEMPLATES"
. poky/oe-init-build-env ${BUILDDIRECTORY}
